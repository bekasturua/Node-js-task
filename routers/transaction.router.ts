...
transactionRouter.get("/transactions", [
    ...
    try {
        ...
        const { type, amount, from, to, orderBy, status, order } = req.query as any;

        const transaction = await getTransactions((req as any).user as User, type, amount, order, orderBy, status, from, to)
