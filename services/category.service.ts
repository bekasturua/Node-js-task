...
export const createCategory = async (name: string, user: User) => {
  ...
  try {
    const category = await db
      ...
      .insertOne({ name, user_id: user._id });
...
export const updateCategory = async (
  ...
  try {
    const category = await db
      .collection('categories')
      .updateOne({ name, user_id: user._id }, { $set: { name: newName } });

    if (category.modifiedCount === 0) {
      throw new Error(
        'Can not find category with the name or it does not belong to this user. ',
      );
    }
...
export const deleteCategory = async (name: string, user: User) => {
  ...
  try {
    ...
    transactions.map(async (transaction) => {
      const categoryIds = transaction.category_ids.map(
        (category_id: ObjectId) => category_id.toString(),
      );
      const categoryToDeleteIndex = categoryIds.indexOf(
      ...
      await db
        ...
        .updateOne(
          { _id: transaction._id },
          { $set: { category_ids: transaction.category_ids } },
        );
    });
